...
import { X, Plus, Trash2, Save, Clock, Type, GripVertical, AlertCircle, FileText } from 'lucide-react';
    ...
    <div className="bg-gray-50 p-4 rounded-lg border border-gray-200 mb-4 animate-fade-in-up cursor-default">
      <div className="grid grid-cols-2 gap-4 mb-4">
        <div>
          ...
          <input value={formData.label || ''} onChange={e => setFormData({...formData, label: e.target.value})} className={`w-full px-3 py-2 border rounded-md text-sm ${errors.label ? 'border-red-500 bg-red-50' : ''}`} placeholder="例如: 早班" />
          {errors.label && <p className="text-xs text-red-500 mt-1">{errors.label}</p>}
        </div>
        <div>
           <label className="block text-xs font-bold text-gray-500 mb-1">簡稱 (顯示用) <span className="text-red-500">*</span></label>
           <input value={formData.shortLabel || ''} onChange={e => setFormData({...formData, shortLabel: e.target.value})} className={`w-full px-3 py-2 border rounded-md text-sm ${errors.shortLabel ? 'border-red-500 bg-red-50' : ''}`} placeholder="例如: 早" maxLength={4} />
        ...
        <div>
          ...
          <input value={formData.time || ''} onChange={e => setFormData({...formData, time: e.target.value})} className="w-full px-3 py-2 border rounded-md text-sm" placeholder="例如: 09:00 - 17:30 或 1300-1700" />
        </div>
        <div className="grid grid-cols-2 gap-2">
            <div>
               <label className="block text-xs font-bold text-gray-500 mb-1">正班時數</label>
               <input type="number" step="0.5" min="0" value={formData.hours || 0} onChange={e => setFormData({...formData, hours: parseFloat(e.target.value)})} className="w-full px-3 py-2 border rounded-md text-sm" />
            </div>
            <div>
               <label className="block text-xs font-bold text-red-500 mb-1">預設加班</label>
               <input type="number" step="0.5" min="0" value={formData.defaultOvertime || 0} onChange={e => setFormData({...formData, defaultOvertime: parseFloat(e.target.value)})} className="w-full px-3 py-2 border rounded-md text-sm text-red-600 font-bold bg-red-50" />
            </div>
        </div>
      </div>
      <div className="mb-4">
        ...
        <div className="relative">
          ...
          <input value={formData.description || ''} onChange={e => setFormData({...formData, description: e.target.value})} className="w-full pl-9 pr-3 py-2 border border-gray-300 rounded-md text-sm focus:ring-1 focus:ring-brand-500 outline-none" placeholder="例如: 含0.5小時休息時間..." />
        </div>
      </div>
      <div className="mb-4">
        ...
        <div className="flex flex-wrap gap-2">
          {COLOR_PRESETS.map((preset) => (
            <button key={preset.name} onClick={() => setFormData({...formData, color: preset.color})} className={`w-8 h-8 rounded-full border-2 transition-all ${preset.color.replace('text-', 'bg-').split(' ')[0]} ${formData.color === preset.color ? 'border-gray-600 scale-110 shadow-md' : 'border-transparent'}`} title={preset.name} />
          ))}
        </div>
      </div>
      <div className="flex justify-end gap-2">
        <button onClick={() => { setEditingCode(null); setErrors({}); }} className="px-4 py-2 text-gray-600 hover:bg-gray-200 rounded text-sm">取消</button>
        <button onClick={handleSave} className="px-4 py-2 bg-brand-600 text-white rounded hover:bg-brand-700 text-sm flex items-center gap-2"><Save size={16} /> 儲存</button>
      </div>
    </div>
  );
    ...
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center backdrop-blur-sm p-4">
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col">
        <div className="bg-gray-50 border-b border-gray-200 px-6 py-4 flex justify-between items-center rounded-t-xl">
          <h2 className="text-gray-800 text-lg font-bold flex items-center gap-2"><Clock size={20} /> 管理班別設定</h2>
          <button onClick={onClose} className="text-gray-400 hover:text-gray-600"><X size={24} /></button>
        </div>
        <div className="p-6 flex-1 overflow-y-auto bg-white">
          <div className="space-y-3 mb-6">
             {sortedShifts.map((shift, index) => (
                <div key={shift.code} draggable={editingCode === null} onDragStart={(e) => { dragItem.current = index; e.currentTarget.classList.add('opacity-50'); }} onDragEnter={(e) => { dragOverItem.current = index; e.preventDefault(); }} onDragEnd={handleDragEnd} onDragOver={(e) => e.preventDefault()} className={`transition-all ${editingCode === null ? 'cursor-move' : ''}`}>
                  {editingCode === shift.code ? renderForm() : (
                    <div className="flex items-center justify-between p-3 border border-gray-100 rounded-lg hover:border-brand-200 hover:shadow-sm transition-all group bg-gray-50">
                       <div className="flex items-center gap-3 flex-1">
                          ...
                          <div className={`w-12 h-10 rounded flex items-center justify-center text-xs font-bold ${shift.color} shadow-sm flex-shrink-0`}>{shift.shortLabel}</div>
                          <div>
                             <div className="font-bold text-gray-800 text-sm flex items-center gap-2">
                               {shift.label} <span className="text-xs font-normal text-gray-500 bg-white px-1.5 py-0.5 rounded border border-gray-200">{shift.hours}h</span>
                             ...
                             <div className="flex items-center gap-2 text-xs text-gray-500">
                               ...
                               {shift.description && <span className="text-gray-400 border-l pl-2 border-gray-300 truncate max-w-[150px]">{shift.description}</span>}
                             </div>
                          </div>
                       </div>
                       <div className="flex gap-2 opacity-100 sm:opacity-0 group-hover:opacity-100 transition-opacity">
                          <button onClick={() => handleEdit(shift)} className="p-1.5 text-gray-400 hover:text-brand-600 hover:bg-brand-50 rounded"><Type size={16} /></button>
                          <button onClick={() => handleDelete(shift.code)} className="p-1.5 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded"><Trash2 size={16} /></button>
                       </div>
                    </div>
                  )}
                </div>
          ...
          {editingCode !== 'NEW' && (
             <button onClick={handleCreate} className="w-full py-3 border-2 border-dashed border-gray-300 rounded-lg text-gray-500 hover:border-brand-400 hover:text-brand-600 hover:bg-brand-50 transition-all flex items-center justify-center gap-2 font-medium">
               <Plus size={20} /> 新增班別
             </button>
          )}
        </div>
        <div className="bg-gray-50 border-t border-gray-200 px-6 py-4 flex justify-end rounded-b-xl">
           <button onClick={onClose} className="px-6 py-2 bg-gray-900 text-white rounded-lg hover:bg-gray-800 font-medium text-sm shadow-sm">完成</button>
        </div>
      </div>
    </div>
  );
